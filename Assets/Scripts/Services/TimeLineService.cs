...
public class TimeLineService : MonoBehaviour
{
    ...
    public void CreateTime()
    {
        ...
        {
            TimeList.Add(Instantiate(TimeElementPB, new Vector3(-4.5f, 5 + (0.5f * i * GameSpeed), 0), Quaternion.identity, transform));
            TimeList[i].GetComponent<TextMesh>().text = $"T {0.5f * i}" + " - " + (TimeList[i].transform.position.y).ToString();
        }
